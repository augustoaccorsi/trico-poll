...
async function main(): Promise<void> {
  const groupJids = parseGroupJids(process.env.WA_TEST_GROUP_JID)
  if (groupJids.length === 0) {
    logger.error('WA_TEST_GROUP_JID is not set')
    process.exit(1)
  }

  logger.info('Fetching probabilities from UFMG (test)')
  ...
  logger.info('Test probabilities sent. Exiting.')
